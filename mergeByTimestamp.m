...
while traceIndex <= numel(traceTimestamps)
    ...
    trans_x(traceIndex) = weightedAvg(behavData,...
        [prevBehavIndex behavIndex], 'trans_x', avgWeights);
    trans_y(traceIndex) = weightedAvg(behavData,...
        [prevBehavIndex behavIndex], 'trans_y', avgWeights);
...
resultTable = table(traceTimestamps', trans_x, trans_y, inside_roi, ...
    dist_reward0, dist_reward1, trace', 'VariableNames', ...
    {'timestamp', 'trans_x', 'trans_y', 'inside_roi', ...
