...
down.rewdist %>%
  ...
  geom_vline(xintercept = rew.dist.threshold, linetype='dotted') +
  coord_flip() +
...
summary.df = summarize.downrew.pc.pct(down.rewdist, dist.threshold=rew.dist.threshold) %>%
...
dca1.rew.pc.pct = subset(summary.df, implant == 'dCA1' & animal_beforetest_day %in% kept.testtrials$animal_beforetest_day & animal != 'C-1R')
vca1.rew.pc.pct = subset(summary.df, implant == 'vCA1' & animal_beforetest_day %in% kept.testtrials$animal_beforetest_day)
...
```
Change in the number pct of place cells not significant
```{r}
summary.df %>%
  filter(animal_beforetest_day %in% kept.testtrials$animal_beforetest_day) %>%
  filter(animal != 'C-1R') %>%
...
t.test(pct.reward ~ exp_title, dca1.rew.pc.pct, paired=T)
...
ggsave('/tmp/downsampled_dist_multifield.svg', units='cm', width=22, height=22)
```

```{r}
peaks.summary.df = summarize.downrew.pc.pct(down.peaks.dist, dist.threshold=rew.dist.threshold) %>%
  dplyr::mutate(animal_beforetest_day = paste(animal, day_desc, sep='_'))

dca1.rew.pc.pct = subset(peaks.summary.df, implant == 'dCA1' & animal_beforetest_day %in% kept.testtrials$animal_beforetest_day & animal != 'C-1R')
vca1.rew.pc.pct = subset(peaks.summary.df, implant == 'vCA1' & animal_beforetest_day %in% kept.testtrials$animal_beforetest_day)
fem.dca1 = lmerTest::lmer(pct.reward ~  exp_title + (1 + day_desc | animal), data=dca1.rew.pc.pct)
print('Fraction of dCA1 place cells at reward')
summary(fem.dca1)
plot.model.diagnostics(fem.dca1, dca1.rew.pc.pct$animal, dca1.rew.pc.pct$exp_title)
