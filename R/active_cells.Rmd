...
for (caimg_result_dir in caimg_result_dirs) {
  ...
  data.traces[, trial_name := paste(trial.meta.row$short_day_desc[1], exp_title, num2str(trial, fmt=0))]
  cell.events.df = data.traces[, .(day.nevents=sum(is.event)), by=.(animal, date, cell, cell_id)]
  
  trial.activity.df = data.traces[, 
    .(event.rate = sum(is.event) / .N * fr,
...
cell.day.activity.df$exp_title = factor(cell.day.activity.df$exp_title,
                                        levels = c('homecage', 'trial', 'aftertest','beforetest'))
...
for (caimg_result_dir in caimg_result_dirs) {
  ...
  timebinned.traces[, mobile := ifelse(is_running, 'mobile', 'immobile') ]
