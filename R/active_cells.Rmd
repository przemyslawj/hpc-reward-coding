...
fst.learning.day = as.Date('2019-07-23')
...
ggplot(data=summary.cell.overlap) +
  geom_line(aes(x=days.diff, y=recall.mean, group=animal, color=animal)) +
  geom_ribbon(aes(x=days.diff, ymin=recall.mean - recall.sem, 
                  ymax=recall.mean + recall.sem, group=animal, fill=animal), alpha=0.2) +
...
```{r}
active.day.df %>%
  filter(cell.active, date >= fst.learning.day) %>%
