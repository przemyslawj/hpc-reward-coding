...
```{r}
data_dir = '/mnt/DATA/Prez/cheeseboard/2019-07-habituation/'
#data_dir = '~/neurodata/cheeseboard/2019-02-learning/'
ca_img_result_dir = paste0(data_dir, '/')
subject = '1BR'
gen_imgs_dir = '/tmp/'
...
first_learning_day = char2date('2019-02-20')
last_day = max(char2date(data$date))
max_trial = max(data$trial)
data = mutate(data, 
              learning_day = as.integer(char2date(date) - first_learning_day + 1),
...
notest.day.summary = day.summary.df %>%
  ...
  mutate(learning_day = as.integer(char2date(date) - first_learning_day + 1))
...
rew.aligned.data$date = char2date(rew.aligned.data$date)
...
```{r}
day.summary.df %>%
  mutate(learning_day = as.integer(char2date(date) - char2date('2019-02-20') + 1)) %>%
