...
```{r}
events.df %>%
  filter(date=='2019-03-08', is_test == FALSE) %>%
...
day.summary.df = group_by(rew_responsive.df, date, cell, is_test) %>%
  summarise(nevents_at_rew=sum(is_at_rew & (nevents > 0), na.rm = TRUE),
            pct_events_at_rew0=sum(is_at_rew0 & (nevents > 0), na.rm = TRUE) / sum(nevents, na.rm = TRUE),
            pct_events_at_rew1=sum(is_at_rew1 & (nevents > 0), na.rm = TRUE) / sum(nevents, na.rm = TRUE),
...
ggplot(notest.day.summary, aes(x=factor(1), fill=nrew_responsive)) +
  geom_bar(width=1) +
...
aligned.data.df = filter(data, is_test == FALSE) %>%
         group_by(trial_id) %>%
         mutate(at_rew0 = is.at.reward(velocity, dist_reward0, timestamp, running.thresh = 3),
                at_rew1 = is.at.reward(velocity, dist_reward1, timestamp, running.thresh = 3)) %>%
...
```{r}
rew0.aligned.data = filter(aligned.data.df, timestamp.rew0 > -5000, timestamp.rew0 < 20000)

rew0.aligned.data = rew0.aligned.data %>%
  mutate(trace_id = paste0(trial_id, '_', cell)) %>%
  filter(date=='2019-03-08', trial==2) 

frame.length.ms = (max(rew0.aligned.data$timestamp, na.rm = TRUE) - min(rew0.aligned.data$timestamp, na.rm = TRUE)) /
  (filter(rew0.aligned.data, cell == 1) %>% nrow)
frame.length.ms = round(frame.length.ms)

rew0.aligned.data = rew0.aligned.data %>%
  group_by(trace_id) %>%
  arrange(timestamp.rew0) %>%
  mutate(aligned_timestamp = row_number() * frame.length.ms)

ggplot(rew0.aligned.data, aes(x=aligned_timestamp / 1000 - 5, y=factor(trace_id))) +
...
day.summary.df %>%
  ...
  #filter(learning_day >= 12, (learning_day <17 | is_test == TRUE)) %>%
