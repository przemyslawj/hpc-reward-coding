...
``` {r}
source('~/mnt_code/openv_mouse_tracker/analyse/locations.R')
...
```{r}
geom_rewards = function(locations.df, subject, day, valence='Positive') {
  reward.df = filter(locations.df, date==day, Animal==subject, valence == valence)
  geom_point(data=reward.df, aes(x=trans_x,y=-trans_y), shape=0,color='blue', fill='white', size=7, stroke=5) 
}
```
...
show_trial_events = function(data.df) {
