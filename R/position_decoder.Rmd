...
eval.parts = lapply(caimg_result_dirs, function(caimg_result_dir) {
  ...
  if (nrow(binned.traces) == 0) {
    ...
    binned.traces2 = stimbin.traces(binned.traces2, angle, 45) %>%
      mutate(bin.angle = pmin(8,bin.angle + 5))
...
for (i in 1:length(caimg_result_dirs)) {
  ...
  if (i > 1 && animal_names[i] == animal_names[i-1]) {
    ...
    system.time(eval.res <- eval.testdata2(binned.traces.filtered, models[[i-1]], classifier.fun=bayesmax))
  ...
  models[[i]] = create.discrete.bayes(filtered.dfs$train, nstim.bins=xybins^2)
}
```
