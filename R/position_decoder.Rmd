...
eval.decoder.all.other.days = function(decoder.config, 
  ...
  for (animal_name in unique(test.days.df$animal)) {
    ...
    binned.traces = prepare.traces(data.traces,
                                   ...
                                   get.bin.thresholds.fun = get.quantiles.fun(c(0.9, 1.0)),
                                   binned.var=binned.var)
    traces.dates = char2date(unique(data.traces$date)) %>% sort()
    cells.present = binned.traces[, (1), by=.(date, cell_id)][, .(ntimes=.N), by=.(cell_id)][
        ntimes == ndays, cell_id]
...
```{r include=FALSE}
beforetest.onedayout.reward.training = eval.decoder.all.other.days.debug(
  bayes.atreward.decoder.config, 
  test.days.df,
  equal.prior = TRUE,
...
```{r include=FALSE}
beforetest.onedayout.reward = eval.decoder.all.other.days.debug(
  bayes.atreward.decoder.config, 
  test.days.df,
  #ncells = 0,
  #nshuffles = 1,
  ...
  #   add.close2.rew = add.close2rew.fun(goal.cell.max.dist)
  #   add.close2.rew(df[timestamp <= 120000])
  # },
  filter.train.traces.fun = function(binned.traces) {
    binned.traces[pmin(dist_reward0, dist_reward1) <= 15/perc2dist | pmin(dist_reward0, dist_reward1) >= 40 / perc2dist, ]
  },
...
beforetest.pop.oneydayout.reward = eval.decoder.all.other.days.debug(
...
caret.atreward.decoder.config = list(
  ...
  predict.fun = caret.svm.predict.debug,
  error.fun = function(x,y) {abs(x-y)}
)
```
...
```{r include=FALSE}
beforetest.onedayout.cell.reward.svm = eval.decoder.all.other.days.debug(
...
```{r include=FALSE}
beforetest.onedayout.reward.svm = eval.decoder.all.other.days.debug(
  caret.atreward.decoder.config, 
  test.days.df,
  #ncells = 0,
  #nshuffles = 1,
