    ...
    for (shuffle_i in 1:nshuffles) {
      ...
      for (i in test.day.indecies) {
        ...
        if (nrow(test.traces) == 0) {
          print('Test traces empty, skipping')
          next
        }
...
beforetest.pop.oneydayout.reward = eval.decoder.all.other.days.debug(
  ...
  test.days.df,
  equal.prior = TRUE,
  prepare.vars.fun = avg.pop.activity.debug,
  filter.train.traces.fun = function(binned.traces) {
...
ggsave('at_rew_pop_classifier.pdf', g, device=cairo_pdf,
       path='/home/prez/tmp/cheeseboard/', units='cm', width=3.0, height=3.8)
```
...
wide.traces.df = tidyr::pivot_wider(
  ...
  names_from='cell_id',
  names_prefix='cell_',
  values_from='zscored_smooth_deconv_trace')
...
gbmFit1 <- train(close2rew ~ ., 
                 #close2rew ~ response_bin * cell_id,
                 data = training, 
                 method = 'svmLinear',
                 #method = 'gbm', 
...
```
TODO: write eval.decoder.other.days.on.traces using carat
