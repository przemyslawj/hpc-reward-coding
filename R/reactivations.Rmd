...
example.traces[(is_running > 0) | reactivation_event,
               .(max.index=which.max(deconv_trace),
                 max.timestamp=timestamp[which.max(deconv_trace)]),
               by=.(date, animal, trial, trial_id, atReward, cell_id)] %>% glimpse
# Hmm, should create events tibble instead: need, index_start, index_end, timestamp_start, timestamp_end

max.activity.aligned = group_by(example.traces, date, animal, exp_title, trial, trial_id) %>%
...
#TODO should compare the cell only ones: from the first time the cell was reactivated
min.deconv.activity = 1e-5
compared.change.df = all.change.combined %>%
  filter(trials.diff <= 7) %>%
  #filter(total_events_trial.1 > 0, total_events_trial.2 > 0) %>%
  ...
  #filter(deconv.95p.ratio > 0, deconv.95p.ratio < Inf) %>%
...
compared.change.df %>%
  ...
  ylab('Trial n+x / Trial n peak activity') + 
  #ylim(c(0.4,1.2)) +
  scale_y_log10(limit=c(0.3, 3), breaks=c(0.4, 0.6, 0.8, 1.0, 2.0, 3.0)) +
  #scale_y_log10(limit=c(0.5, 2)) +
  #coord_trans(y='log10')+
...
reactivations.activity.change.joined %>%
  ...
  geom_hline(yintercept = 1.0, linetype='dashed', color='#444444') +
  geom_jitter(shape=1, alpha=0.4, size=0.4, colour=reactiv.colours[2]) +
  ...
  facet_grid(.~ group) +
  gtheme.light.panel +
  xlab('Reactivations') + ylab('Relative activity peak')
...
reactivated.effect.deconv.ratio = reactivations.activity.change.joined %>%
    #filter(!reactivated_inbetween) %>%
  filter(group == 'learning') %>%
...
lmer.test.print(left_join(reactivated.effect.deconv.ratio, d.day),
                var=deconv.ratio.diff,
                fixed.effects = mean.logdist,
                randef.str = '(1 | animal)',
                diagnostics.groupvar = animal)

g = left_join(reactivated.effect.deconv.ratio, d.day) %>% 
  ggplot(aes(x=mean.logdist, y=deconv.ratio.diff, text=paste(animal, day_desc))) +
  geom_point() +
...
g = left_join(reactivated.effect.deconv.ratio, filter(d, trial_n==8)) %>%
...
lmer.test.print(left_join(reactivated.effect.deconv.ratio, filter(d, trial_n==8)),
                var=deconv.ratio.diff,
