...
cell_names = c(120, 30, 246, # reactivated
               98, 146, 308)# not reactivate)
...
trace.window.len = 12  # 5 sec window
...
bind_rows(
  ...
  facet_grid(cell_id ~ trial + atReward, scales='free_y') +
  gtheme
```
...
# TODO: should only consider traces before left the new reward
...
sem_summary <- function(x, log.transform=FALSE) {
...
compared.change.df %>%
  ...
  stat_summary(aes(colour=reactivated),
               fun.data=sem_summary, 
               mult=1, 
               geom='pointrange', size=0.5, shape=1, log.transform=FALSE) +
  ylab('Trial n+x / Trial n peak activity') + 
  #ylim(c(0.4,1.2)) +
  #scale_y_log10(limit=c(0.03, 3), breaks=c(0.4, 0.6, 0.8, 1.0, 2.0, 3.0)) +
  scale_y_log10() +
  #coord_trans(y='log10')+
