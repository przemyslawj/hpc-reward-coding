...
all.change.combined = all.change.combined %>%
  ...
  ungroup() %>%
  group_by(group, group_desc, reactiv.at.new.rew, implant, animal, date, exp_title, cell_id, trials.diff) %>%
...
x.reactivated.location %>%
  ...
  dplyr::summarise(reactiv.pct.new = mean(new.rew.reactivation.TRUE, na.rm=T),
                   reactiv.pct.old = mean(new.rew.reactivation.FALSE, na.rm=T)) %>%
...
filter(x, reactivated, group=='learning') %>%
  ggplot(aes(x=fst.reactivated.trial, fill=reactiv.at.new.rew)) +
...
x.fst.trial = x.next.trial.reactiv %>%
  #filter(aligned.reactiv.trial %in% c(2))  %>%
  identity()
