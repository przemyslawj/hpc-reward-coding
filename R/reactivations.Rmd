...
all.trial.events.run %>%
  filter(reactivation.rew.loc=='both') %>%
  filter(max.deconv.norm > 0) %>%
  filter(fst.reactivated.trial %in% c(1, 4) | is.infinite(fst.reactivated.trial)) %>%
  mutate(fst.reactivated.trial.noninf = ifelse(is.infinite(fst.reactivated.trial), 8, fst.reactivated.trial)) %>%
...
all.change.combined = all.change.combined %>%
  ...
  dplyr::mutate(fst.reactivated.trial = min(c(trial.denominator[which(reactivated)], Inf)),
                ...
                reactivation.ordinal = ifelse(reactivated,
...
x.reactivated.location.summary = x.reactivated.location %>%
  ...
  dplyr::summarise(ncells = n(), 
                   new = mean(reactivation.loc.new),
                   previous = mean(reactivation.loc.old),
...
x.fst.trial.summary = group_by(x.fst.trial, group, group_desc, reactivated, reactivated_later, trial_n) %>%
  dplyr::summarise(as_tibble(t(sd_summary(log(!!compared.var))) %>% exp),
                             ncells=n(), .groups='drop') 
...
all.change.combined %>%
  ...
  mutate(reactivation.ordinal.char = as.character(reactivation.ordinal),
         trial.denominator.char = as.character(trial.denominator)) %>%
  filter(trial.denominator > 1) %>%
  ggplot(aes(x=reactivation.ordinal.char, y=max.deconv.ratio)) +
  geom_jitter(height=0, width=0.1, shape=1, alpha=0.4) +
  geom_violin(fill='transparent', draw_quantiles = 0.5) +
  facet_wrap(trial.denominator.char ~ ., scales='free') +
  scale_y_log10() +
  gtheme.light.panel
```
...
x.compared = x %>%
  ...
  filter(aligned.reactiv.trial == 3) %>%
...
x.compared %>%
  ...
  scale_x_continuous(breaks=-1:5, limit=c(-1.3, 5.3)) +
  scale_color_manual(values=reactiv.colours) +
...
ggsave('~/tmp/tanjas/trial2trial_change_epsp_like.png', units = 'cm',
       width = 12, height = 55.3)
```
...
```{r}
all.foraging.trial1.events.run = all.foraging.trial1.events %>%
  group_by(animal, date, exp_title, cell_id, trial_minute) %>%
  dplyr::summarise(reactivation_events = total_reactivation_events[2],
                   total_events = total_events[1],
                   max.deconv = max.deconv[1],
                    .groups='drop') %>%
...
all.foraging.trial1.events.run = all.foraging.trial1.events.run %>%
  arrange(animal, date, exp_title, cell_id, trial_minute) %>%
  group_by(animal, date, exp_title, cell_id) %>%
  mutate(max.deconv.norm = max.deconv / max(max.deconv),
         fst.reactivated.trial = min(c(trial_minute[which(reactivated)], Inf))) %>%
  ungroup() %>% as.data.table() 

all.foraging.trial1.events.run %>%
  filter(max.deconv.norm > 0) %>%
  filter(fst.reactivated.trial %in% c(0, 4) | is.infinite(fst.reactivated.trial)) %>%
  mutate(fst.reactivated.trial.noninf = ifelse(is.infinite(fst.reactivated.trial), 8, fst.reactivated.trial)) %>%
  group_by(exp_title, fst.reactivated.trial.noninf, trial_minute) %>%
  dplyr::summarise(mean.max.deconv.norm = mean((max.deconv.norm), na.rm=TRUE),
                   sem.max.deconv.norm = sem((max.deconv.norm)),
                   .groups='drop') %>%
  ggplot(aes(x=trial_minute, 
             y=mean.max.deconv.norm, 
             group=fst.reactivated.trial.noninf, 
             fill=fst.reactivated.trial.noninf,
             colour=fst.reactivated.trial.noninf)) +
