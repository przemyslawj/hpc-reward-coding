...
#uniform.pc.dist = calc.rew.distances.df(filter(test.rewards.df, exp_title=='beforetest'),
#                                        uniform.xycoords$field.x, uniform.xycoords$field.y) %>%
#  left_join(test.rewards.df, by=c('animal', 'location_set')) %>%
#  dplyr::rename(day_desc.reward = day_desc)
...
trial.peaks.at.rew %>%
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
Distribution of min distance to peak 
```{r}
trial.peaks.at.rew %>% 
  filter(day_desc == 'learning1 day#5') %>%
  ggplot(aes(x=peak2rew.mindist, color=signif.si, group=signif.si)) +
  geom_density() + 
  #stat_summary(aes(y=0.0), fun.x=median, geom="point", shape=23, size=2) +
  facet_wrap(implant + animal ~ ., ncol=4) +
