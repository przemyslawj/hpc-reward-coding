...
for (animal_name in unique(pc.test.df$animal)) {
  all.days = unique(subset(pc.test.df, animal==animal_name)$date) %>% as.Date
...
habit.fields.at.test.rews = map_dfr(1:nrow(habittest.daypair), ~ {
  ...
  activity_date = as.character(habittest.daypair$date.habitday[.x])
  test_date = as.character(habittest.daypair$date.testday[.x])
  rew.df = beforetest.rewards.df[animal==animal_name & date == test_date,]
  calc.field.peaks.info(activity_date,
                        animal_name,
                        run.fields[[animal_name]][[activity_date]], 
                        rew.df=rew.df,
...
beforetest.peaks.at.rew %>% 
  ...
  dplyr::select(animal, cell_id, day_desc, signif.si) %>%
  reshape2::dcast(animal + cell_id ~ day_desc, fun.aggregate=mean) %>% 
  dplyr::group_by(animal) %>%
  dplyr::summarise(l2d1.n = sum(!is.na(`learning2 day#1`)),
                   nsignif.l2d1.n = sum(`learning2 day#1`, na.rm=T),
                   recall.l3d1.pct = mean(!is.na(`learning3 day#1`)) * 100,
                   recall.l3d1.n = sum(!is.na(`learning3 day#1`)),
                   recall.l3d1.nsignif = sum(`learning2 day#1` & !is.na(`learning3 day#1`)),
                   recall.l3d3.pct = mean(!is.na(`learning3 day#3`)) * 100,
                   recall.l3d3.n = sum(!is.na(`learning3 day#3`)),
                   recall.l3d3.nsignif = sum(`learning2 day#1` & !is.na(`learning3 day#3`))) %>% 
  dplyr::mutate_if(is.double, round, 0) %>%
  DT::datatable()
