...
create.hist.tibble = function(vals) {
  h = hist(vals, breaks=hist.breaks, plot=FALSE) 
  tibble(count=h$counts, mid=as.integer(h$mids))
}
...
shuffled.cell.dist.hist.summary = shuffled.cell.dist.summary %>%
  ...
  dplyr::summarise(m.count=mean(count), sem.count=sem(count), sd.count=sd(count))
...
#ggplot(data=cell.rew.distance.summary, aes(x=m.dist)) +
  #geom_density(data=shuffled.cell.dist.summary, linetype='dashed', color='grey60') +
