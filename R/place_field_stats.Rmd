...
beforetest.peaks.at.rew %>% 
  left_join(trials.meta.df) %>% 
  dplyr::mutate(animal_cell_id = paste(animal, cell_id, sep='_')) %>%
  dplyr::mutate(location_set_name = paste0('loc', location_set-1)) %>%
  dplyr::filter(animal_cell_id %in% pc_cell_ids) %>%
  dplyr::select(animal, cell_id, location_set_name, signif.si) %>%
  reshape2::dcast(animal + cell_id ~ location_set_name, fun.aggregate=mean) %>% 
  dplyr::group_by(animal) %>%
