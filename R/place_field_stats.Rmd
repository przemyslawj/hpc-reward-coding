...
run.trials.si %>%
  ...
  dplyr::summarise(pct.mi.signif=sum(signif.mi)/n()*100) %>%
  ggplot() +
  geom_boxplot(aes(x=date, y=pct.mi.signif)) +
  gtheme +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
run.trials.si %>%
  ...
  dplyr::arrange(date) %>%
  ddply(.(animal), mutate, active.ratio=round(nactive/nactive[[1]]*100)) %>% 
  select(date, animal, active.ratio) %>%
  ggplot() +
  geom_jitter(aes(x=date, y=active.ratio, color=animal), height=0, width=0.2) +
...
is.place.cell = function(signif.mi, mutual.info.wo.bias, field.size.50, field.size.25, field.max, mfr) {
  signif.mi & 
    mutual.info.wo.bias >= mi.thresh &
    field.size.50 <= field.size.thresh.50 &
    field.size.25 <= field.size.thresh.25 &
    field.max - mfr >= field.peak.thresh
}

# Classification of place cells based on MI on each subset of day sessions
run.trials.pc = run.trials.si %>%
  mutate(is.pc = is.place.cell(signif.mi, mutual.info - mutual.info.bias, field.size.50, field.size.25, field.max, mfr))
...
plot.field.locs = function(pc.df, rewards.day) {
  pc.df %>%
    ggplot() +
    geom_jitter(aes(x=field.max.x, y=100-field.max.y, size=mutual.info-mutual.info.bias, color=min.rew.dist <= goal.cell.max.dist), 
                alpha=0.1, width=1, height=1) +
...
summary.habit.trials.pc = habit.run.trials.pc %>%
  ...
  dplyr::summarise(pct.reward=sum(min.rew.dist <= dist.thr) / n() * 100)

summary.pc.test.df = pc.test.df %>%
  group_by(date, animal) %>%
  dplyr::summarise(at.reward=sum(min.rew.dist <= dist.thr),
...
```{r}
max.xy = getNBinsXY()
daypair.diff = function(animal.daypair.df, days) {
...
for (i in 1:ncol(day.comb)) {
  ...
  for (animal_name in unique(pc.rew.dist.df$animal)) {
    ...
    animal.daypair.df = pc.rew.dist.df %>%
      filter(animal_name == animal, date == days[1] | date == days[2])
...
```{r}
pc.change.df = pc.change.df %>%
  filter(n>1) 

#DT::datatable(pc.change.df)
pc.change.df %>%
  #filter(pc.fst) %>%
  filter(same.rew.loc) %>%
  group_by(animal, days.diff) %>%
  dplyr::summarise(pct.unchanged=sum(mean.pc == 1.0) / n() * 100) %>%
  ggplot() +
...
pc.change.df %>%
  ...
  filter(pc.fst) %>%
  ggplot() +
  geom_violin(aes(x=date.fst, y=field.xy.dist, color=mean.pc==1)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(color='Place cell\non both days', x='Reference day', y='Field move (%)') +
  geom_hline(aes(yintercept=dist.thr), linetype='dashed')
```
...
pc.nextday.unchanged = pc.change.df %>%
  ...
  #filter(stage == 'Habituation' || date.snd %in% fst.learning.days)
...
pc.nextday.unchanged %>%
  ...
  labs(y='Field moved distance', x='')
```
No dfference in changes in place field median distance shift during learning vs during habituation
Stats issue: n=cell count 
```{r}
wilcox.test(field.xy.dist ~ stage, data=pc.nextday.unchanged)
```
...
pc.change.df %>%
  ...
  filter(pc.fst) %>%
  ggplot() +
  geom_violin(aes(x=date.fst, y=field.cor, color=mean.pc==1)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
pc.nextday.changed = pc.change.df %>%
  filter(days.diff==1) %>%
  filter(mean.pc >= 0.5, pc.fst) %>%
  filter(!same.rew.loc)

pc.nextday.changed %>%
 ggplot(aes(x=1, y=field.cor, color=mean.pc==1)) +
  geom_violin() +
  stat_summary(fun.y=median, geom="point", shape=23, size=2) +
  labs(y='Field correlation', x='', color='Place cell\non both days')
```
