...
run.trials.si %>%
  ...
  ggplot() +
  geom_point(aes(x=day_desc, y=pct.mi.signif, colour='MI')) +
  geom_point(aes(x=day_desc, y=pct.si.signif, colour='SI')) +
  gtheme +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ylab('Significant information cells (%)')
```
...
run.trials.si %>%
  ...
  select(date, day_desc, animal, active.ratio) %>%
  ggplot() +
  geom_jitter(aes(x=day_desc, y=active.ratio, color=animal), height=0, width=0.1) +
...
place.cell.db = run.trials.pc %>%
  select(date, animal, cell_id, implant, is.pc, 
         spatial.information, mutual.info, mutual.info.bias, 
         sparsity, field.size.50, field.size.25,
         early.late.cor) %>%
  ungroup()

place.cell.db %>% group_by(implant) %>% dplyr::summarise(pct.pc=sum(is.pc)/n() * 100)
```
...
pc.rew.dist.df = merged.run.trials.pc %>%
  ...
  ddply(.(date, day_desc, animal, cell_id), mutate, 
  ...
  mutate(is.goal.cell = is.pc & (min.rew.dist <= goal.cell.max.dist)) %>%
  replace_na(is.goal.cell=FALSE)

pc.rew.dist.df %>%
  filter(exp != 'habituation') %>%
  ggplot() +
  geom_violin(aes(x=day_desc, y=min.rew.dist, color=implant)) +
  gtheme +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
goal.cell.db = pc.rew.dist.df %>%
  select(animal, date, day_desc, cell_id, is.goal.cell, closest.location.ordinal) %>%
...
goal.cells.df %>%
  ...
  ggplot() +
  geom_point(aes(x=day_desc, y=pct.goal, color=animal)) +
  gtheme +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
habit.run.trials.pc = merged.run.trials.pc %>%
  filter(is.pc) %>%
  left_join(test.rewards.df, by=c('animal', 'implant'), suffix=c('.activity', '.reward')) %>%
...
pc.test.df = test.trials.si %>%
  mutate(is.pc = is.place.cell(signif.mi, mutual.info - mutual.info.bias, field.size.50, field.size.25, field.max, field.mean, 1.0))
...
plot.field.locs = function(pc.df, rewards.day_desc) {
  pc.df %>%
    ...
    geom_point(data=filter(test.rewards.df, day_desc==rewards.day_desc), mapping=aes(trans_x, 100-trans_y), color='red') +
    facet_wrap(. ~ animal, ncol=2) +
    xlim(c(0,100)) + ylim(c(0,100)) +
    labs(size='Mutual info', colour='Goal cell', x='X position (a.u.)', y='Y position (a.u.)') 
}

plot.field.locs(filter(pc.test.df, day_desc=='learning2 day#1'), 'learning2 day#1') +
