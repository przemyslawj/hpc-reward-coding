...
plotted.npeaks.summary %>%
  ...
  xlab('Peaks count') + ylab('Place cells (%)') +
  gtheme
...
cell.dist.df = beforetest.peaks.at.current.rew %>%
  dplyr::select(implant, animal, date, cell_id, min.rew.dist, npeaks.at.rew) %>%
...
shuffled.cell.dist.summary = shuffled.cell.dist.df %>%
  ...
  dplyr::summarise(m.dist = mean(min.rew.dist, na.rm=TRUE), 
                   ...
                   npeaks.at.rew=sum(npeaks.at.rew)) %>%
...
shuffled.cell.atrew.hist = shuffled.cell.dist.summary %>%
  group_by(implant, shuffle.id) %>%
  dplyr::summarise(create.hist.tibble(npeaks.at.rew, hist.breaks=seq(-0.5,10,1)))  # histogram per shuffle
  #dplyr::summarise(create.hist.tibble(npeaks.at.rew / (n * 2) * 100, seq(0,100,20)))

# Percentile of cells in the shuffles with few and many fields
shuffled.pct.atrew.95p = shuffled.cell.atrew.hist %>%
  dplyr::mutate(few_fields = mid <= 0, many_fields = mid >= 3) %>%
...
cell.rew.atrew.hist = cell.rew.distance.summary %>%
  group_by(implant) %>%
  dplyr::summarise(create.hist.tibble(npeaks.at.rew, seq(-0.5,10,1)),
  #dplyr::summarise(create.hist.tibble(npeaks.at.rew / (n * 2) * 100, seq(0,100,20)),
                   pct.count.cumsum = cumsum(pct.count))
X = filter(shuffled.pct.atrew.95p, few_fields) %>% left_join(
  filter(cell.rew.atrew.hist, mid==0)) 
...
X = filter(shuffled.pct.atrew.95p, many_fields) %>% left_join(
  dplyr::filter(cell.rew.atrew.hist, mid==2)) %>%
...
shuffled.cell.atrew.hist.summary %>%
  ...
  facet_wrap(. ~ implant) + ylab('Cells (%)') + xlab('rewards with a field (%)') +
  xlim(c(-0.5, 6.5)) +
  gtheme
...
shuffled.cell.atrew.hist = shuffled.cell.dist.summary %>%
  group_by(implant, shuffle.id) %>%
  dplyr::summarise(create.hist.tibble(npeaks.at.rew, hist.breaks=seq(-0.5,20,1)))  # histogram per shuffle

# Percentile of cells in the shuffles with few or many fields at reward
shuffled.pct.atrew.95p = shuffled.cell.atrew.hist %>%
  dplyr::mutate(few_fields = mid <= 1, many_fields = mid >= 8) %>%
  dplyr::group_by(implant, shuffle.id, few_fields, many_fields) %>%
  dplyr::summarise(npeaks.at.rew=sum(pct.count)) %>% 
  dplyr::group_by(implant, few_fields, many_fields) %>%
  dplyr::summarise(npeaks.05p = quantile(npeaks.at.rew, 0.05)[1],
                   npeaks.95p = quantile(npeaks.at.rew, 0.95)[1])
...
cell.rew.atrew.hist.summary = cell.rew.distance.summary %>%
  group_by(implant) %>%
  dplyr::summarise(create.hist.tibble(npeaks.at.rew, seq(-0.5,20,1)),
                   pct.count.cumsum = cumsum(pct.count))
X = filter(shuffled.pct.atrew.95p, few_fields) %>% left_join(
