...
calc.min.rew.dist = function(rew.df, day, animal_name, field.x, field.y) {
  ...
  rews = rew.df[animal == animal_name, ]
  if (!is.null(day) && ('date' %in% colnames(rews.df))) {
    
    rews = rews[date==format(day) ,]  
  }
...
calc.field.peaks.info = function(day, animal_name, 
                                 ...
                                 max.rew.dist.thr=20) {
  rews.df = data.table(rews.df)
  if ('date' %in% colnames(rews.df)) {
    rew.df = rews.df[animal==animal_name & date==day, ]
  } else {
    rew.df = rews.df[animal==animal_name, ]
  }
  if (nrow(rew.df) == 0) {
    return(data.frame())
  }
...
field.peaks.df = map2_dfr(trial.days$date, 
                          trial.days$animal, 
                          ~ calc.field.peaks.info(.x, .y, run.fields))
field.peaks.df$exp_title = 'trial'
```

```{r warning=FALSE}

field.peaks.beforetest = map2_dfr(trial.days$date, 
                                  trial.days$animal, 
                                  ~ calc.field.peaks.info(.x, .y, beforetest.fields))
field.peaks.beforetest$exp_title = 'beforetest'
field.peaks.beforetest$cell_id = as.integer(field.peaks.beforetest$cell_id)
field.peaks.beforetest = left_join(field.peaks.beforetest, trials.meta.df)

field.peaks.aftertest = map2_dfr(trial.days$date, 
                                 trial.days$animal, 
                                 ~ calc.field.peaks.info(.x, .y, aftertest.fields))
field.peaks.aftertest$exp_title = 'aftertest'
field.peaks.aftertest$cell_id = as.integer(field.peaks.aftertest$cell_id)
field.peaks.aftertest = left_join(field.peaks.aftertest, trials.meta.df)
```
...
field.peaks.habituation = map2_dfr(trial.days$date, 
                                   trial.days$animal, 
                                    ~ calc.field.peaks.info(.x, .y, run.fields, rews.df=fst.moved.location.df))
field.peaks.habituation$exp_title = 'habituation'
field.peaks.habituation$cell_id = as.integer(field.peaks.habituation$cell_id)
field.peaks.habituation = left_join(field.peaks.habituation, trials.meta.df) %>%
  left_join(mouse.meta.df)
```
```{r}
field.peaks.habituation %>% 
  left_join(place.cell.db) %>%
  filter(signif.si) -> a
a.hist = hist(subset(a, implant=='dCA1')$rew.peaks.count)
a.hist$counts

a.hist = hist(subset(a, implant=='vCA1')$rew.peaks.count)
a.hist$counts
```
...
field.peaks.df = left_join(field.peaks.df, place.cell.db) 
...
field.peaks.df %>%
  ...
  left_join(subset(field.peaks.df, day_desc=='learning2 day#2'), by=c('implant','animal', 'cell_id'), suffix=c('.fst', '.snd')) %>%
  ...
  dplyr::summarise(median(nmatched),
                   ...
                   sem(two.rews.pct)) %>%
  dplyr::mutate_if(is.double, percent, 1)
```
...
How many cells active at previous reward are still active at that reward after learning?
```{r}
field.peaks.beforetest %>%
  filter(day_desc=='learning2 day#1') %>%
  #filter(animal=='G-BR') %>%
  left_join(pc.test.df) %>% 
  #filter(signif.si) %>%
  dplyr::filter((rew.peaks.count - current.rew.peaks.count) > 0) %>%
  left_join(filter(field.peaks.aftertest, day_desc=='learning2 day#1'), by=c('animal', 'cell_id'), suffix=c('.fst', '.snd')) %>%
...
transloc.rew.pf.change = 
  ...
  #filter(signif.si) %>% 
  dplyr::filter((rew.peaks.count - current.rew.peaks.count) > 0) %>%
  left_join(filter(field.peaks.df, day_desc=='learning2 day#1'), by=c('implant', 'animal', 'cell_id'), suffix=c('.fst', '.snd')) %>%
  dplyr::mutate(old.present = (rew.peaks.count.snd - current.rew.peaks.count.snd) > 0,
                old.disappeared = !old.present,
                old.moved = !old.present & (current.rew.peaks.count.snd > 0),
                peaks.unmoved = old.present & (current.rew.peaks.count.fst == current.rew.peaks.count.snd),
...
```{r}
transloc.rew.pf.change %>%
  left_join(filter(field.peaks.habituation, day_desc == 'habituation day#3') %>% dplyr::select(-date, -day_desc), 
            by=c('animal', 'cell_id')) %>% 
...
filter(field.peaks.habituation, day_desc=='habituation day#3') %>%
  left_join(filter(field.peaks.beforetest, day_desc=='learning2 day#1'),
            by=c('animal', 'cell_id'),
            suffix=c('.habit', '.test')) %>% 
