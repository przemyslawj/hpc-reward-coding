...
Early-late correlation of spatial information
```{r}
run.trials.pc %>%
...
uniform.xycoords = sim.xy.coord(nplace.fields=100)
uniform.fields.df = sim.fields.from.occupied(beforetest.occupancies, min.occupancy = 5)
...
g.test.pf.shift = ggplot() +
  ...
  geom_density(data=filter(pc.test.df, signif.si), aes(x=min.rew.dist)) +
...
day.cors.df %>%
  ...
  group_by(date, animal, signif.si) %>%
...
ggplot() +
  ...
  geom_density(data=filter(beforetest.peaks.fst.moved, signif.si, day_desc == 'learning2 day#1 test'), aes(x=peak2rew.mindist)) +
...
beforetest.peaks.at.rew %>%
  left_join(trials.meta.df) %>%
  filter(day_desc=='learning2 day#1', animal=='D-BR') %>%
  left_join(pc.test.df) %>% 
  filter(signif.si) %>%
  dplyr::filter((rew.peaks.count - current.rew.peaks.count) > 0) %>%
  left_join(subset(field.peaks.df, day_desc=='learning2 day#1'), by=c('implant','animal', 'cell_id'), suffix=c('.fst', '.snd')) %>%
  dplyr::filter((rew.peaks.count.snd - current.rew.peaks.count.snd) > 0) %>% View
```
