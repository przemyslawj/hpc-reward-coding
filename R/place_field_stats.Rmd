...
compared.pcs.df = 
  ...
  place.cell.db %>% filter(exp=='habituation') %>%
    left_join(trials.meta.df) %>%
    left_join(mouse.meta.df) %>%
    filter(animal != 'L-TL') 
    

plot.pc.stat = function(metric) {
  metric = enquo(metric)
  compared.pcs.df %>%
    filter(signif.si) %>%
    ...
    geom_violin(fill='#8eabc0ff', color='#8eabc0ff') +
...
g.place.cell.count = summary.compared.pcs.df %>%
  ...
  #geom_point(shape=1) +
  geom_jitter(shape=1, width=0.15, height=0, size=0.5, color='#222222')+
  gtheme +
  ylab('Place cells (%)') + xlab('') + ylim(c(0,80))
 
plot_grid(
  g.place.cell.count,
  plot.pc.stat(field.size.50 / nbins / nbins * 100) +
    ylab('% of maze'),
  plot.pc.stat(spatial.information) + ylab('bits per dF unit'),
  plot.pc.stat(early.late.cor) + ylab('early vs late correlation'),
  nrow = 1)

ggsave('/tmp/pf_stats.svg', width=14, height=4.3, units='cm')
```

```{r}
print('Percent of place cells not different')
field.size.model = lmerTest::lmer(pc.pct ~ implant + (1 | animal), data=summary.compared.pcs.df, REML=TRUE)
...
field.size.model = lmerTest::lmer(log(field.size.50 / nbins / nbins * 100) ~ implant + (1 + day_desc | animal), 
...
sprintf('Estimate change (log model) by %.0f pct (%.0f; %.0f)', 
        exp(x$Estimate) * 100,
        exp(x$lower) * 100,
        exp(x$upper) * 100)

print('Spatial information not statistically different')
spatial.info.model = lmerTest::lmer(log(0.05 + spatial.information) ~ implant + (1 + day_desc | animal), 
                                    data=compared.pcs.df, REML=TRUE)
...
early.late.cors.model = lmerTest::lmer(early.late.cor ~ implant + (1 + day_desc | animal), 
                                      data=compared.pcs.df, REML=TRUE)
...
plot.model.diagnostics(early.late.cors.model, 
                       subset(compared.pcs.df, !is.na(early.late.cor))$animal, 
                       subset(compared.pcs.df, !is.na(early.late.cor))$implant)
```
...
plotted.npeaks.summary %>%
  ...
  geom_step(data = filter(plotted.npeaks.summary,exp=='learning'),
            aes(x=mid-0.5, y=pct.mean), size=2) +
  facet_wrap(implant ~ .) +
...
plotted.npeaks.summary %>% 
  ...
  geom_errorbar(aes(ymin=pct.mean - pct.sem, ymax=pct.mean + pct.sem), width=0.3) +
  geom_step(aes(x=mid-0.5, y=pct.mean), size=1) +
  scale_color_manual(values=c('#444444', '#0098ff')) +
  xlab('# fields') + ylab('Place cells (%)') +
  gtheme
```
...
multifield.model = lmerTest::lmer(pct.multifield ~ implant * exp + (1 | animal),
...
bind_rows(
  ...
  gtheme +
  xlim(c(0,100)) +
  xlab('Distance to closer reward (cm)') + ylab('Density')
```
...
dca1.rew.pc.pct = subset(merged.summary.df, implant == 'dCA1')
vca1.rew.pc.pct = subset(merged.summary.df, implant == 'vCA1')
fem.dca1 = lmerTest::lmer(pct.reward ~  exp + (1 + exp | animal), data=dca1.rew.pc.pct)
print('Fraction of dCA1 place cells at reward increased')
anova(fem.dca1, refit=FALSE, ddf='Satterthwaite')
summary(fem.dca1)
plot.model.diagnostics(fem.dca1, dca1.rew.pc.pct$animal, dca1.rew.pc.pct$exp)

fem.vca1 = lmerTest::lmer(pct.reward ~  exp + (1 + exp | animal), data=vca1.rew.pc.pct)
print('Fraction of vCA1 place cells did not increase')
summary(fem.vca1)
anova(fem.vca1, refit=FALSE, ddf='Satterthwaite')
plot.model.diagnostics(fem.vca1, vca1.rew.pc.pct$animal, vca1.rew.pc.pct$exp)
...
(Need to be reassessed on downsampled)
```{r}
...
TODO: only include days when the cell was a place cell
...
shuffled.cell.dist.summary = shuffled.cell.dist.df %>%
  ...
  filter(n > 1) %>%  # only include cells present in at least two days
...
TODO: need to normalize by the number of total rewards when the cell was present
```{r}
...
shuffled.cell.atrew.hist = shuffled.cell.dist.summary %>%
  group_by(implant, shuffle.id) %>%
  #dplyr::summarise(create.hist.tibble(rew.peaks.count, hist.breaks=seq(-0.5,10,1)))  # histogram per shuffle
  dplyr::summarise(create.hist.tibble(rew.peaks.count / (n * 2) * 100, seq(0,100,rew.peaks.pct.step)))
...
shuffled.pct.atrew.95p = shuffled.cell.atrew.hist %>%
  ...
  dplyr::summarise(cell.pct.05p = quantile(cell.pct, 0.05)[1],
                   cell.pct.95p = quantile(cell.pct, 0.95)[1])
...
cell.rew.atrew.hist = cell.rew.distance.summary %>%
  group_by(implant) %>%
  #dplyr::summarise(create.hist.tibble(rew.peaks.count, seq(-0.5,10,1)),
  dplyr::summarise(create.hist.tibble(rew.peaks.count / (n * 2) * 100, seq(0,100,rew.peaks.pct.step)),
                   pct.count.cumsum = cumsum(pct.count))
X = filter(shuffled.pct.atrew.95p, few_fields) %>% left_join(
  filter(cell.rew.atrew.hist, mid < few.fields.thr)) 
print(dplyr::select(X, implant, cell.pct.05p, cell.pct.95p, pct.count.cumsum))

X = filter(shuffled.pct.atrew.95p, many_fields) %>% left_join(
  dplyr::filter(cell.rew.atrew.hist, mid < many.fields.thr)) %>%
...
g = shuffled.cell.atrew.hist.summary %>%
  ...
  geom_errorbar(aes(ymin=m.count-sd.count, ymax=m.count+sd.count), width=rew.peaks.pct.step / 3, color='#666666') +
  geom_step(data=cell.rew.atrew.hist.summary, aes(x=mid-rew.peaks.pct.step/2, y=pct.count), color='#0098ff', size=1) +
  facet_wrap(. ~ implant) + ylab('Cells (%)') + xlab("Reward fields (%)") +
  labs(title="Count of cell's reward fields") +
  #xlim(c(-0.5, 6.5)) +
  scale_x_continuous(breaks = seq(rew.peaks.pct.step/2, 100, rew.peaks.pct.step)) +
  #scale_y_log10() +
  gtheme
g
ggsave('/home/prez/tmp/cheeseboard/reward_fields.pdf', g,
       height=5, width=6.5, units='cm', device=cairo_pdf, dpi=300)
```

## Same cells at reward during baited trials
```{r}
pc.ids = filter(trial.peaks.at.current.rew, signif.si) %>%
  dplyr::select(animal, cell_id) %>%
  dplyr::distinct() %>%
  dplyr::mutate(animal_cell = paste(animal, cell_id, sep='_')) 
...
shuffled.cell.dist.df = map_dfr(
  1:nsamplings, ~ dplyr::mutate(cell.dist.df, 
                                shuffle.id=.x,
                                shuffle.cell_id = cell_id[shuffle(length(cell_id))]))
...
shuffled.cell.dist.summary = shuffled.cell.dist.df %>%
  ...
  dplyr::summarise(m.dist = mean(min.rew.dist, na.rm=TRUE), 
                   ...
                   at.rew.times=sum(min.rew.dist <= goal.cell.max.dist),
                   rew.peaks.count=sum(rew.peaks.count)) %>%
  filter(n > 4) %>%  # only include cells present in at least five days
  dplyr::mutate(group='sampled', line_g=paste0(group, shuffle.id))
```
...
shuffled.cell.atrew.hist = shuffled.cell.dist.summary %>%
  group_by(implant, shuffle.id) %>%
  dplyr::summarise(create.hist.tibble(rew.peaks.count, hist.breaks=seq(-0.5,20,1)))  # histogram per shuffle
...
cell.rew.atrew.hist.summary = cell.rew.distance.summary %>%
  group_by(implant) %>%
  dplyr::summarise(create.hist.tibble(rew.peaks.count, seq(-0.5,20,1)),
                   pct.count.cumsum = cumsum(pct.count))
X = filter(shuffled.pct.atrew.95p, few_fields) %>% left_join(
...
shuffled.cell.atrew.hist.summary %>%
  ggplot(aes(x=mid)) +
  geom_bar(aes(y=m.count),  stat='identity', position='dodge', width=1,
           fill='white', color='#444444', alpha=0.5) +
  geom_errorbar(aes(ymin=m.count-sd.count, ymax=m.count+sd.count), width=0.4, color='#444444') +
  geom_step(data=cell.rew.atrew.hist.summary, aes(x=mid-0.5, y=pct.count), color='#0098ff', size=2) +
  facet_wrap(. ~ implant) + ylab('Cells (%)') + xlab('# fields at reward') +
  #xlim(c(-0.5, 6)) +
  gtheme
