...
g.place.cell.count = compared.pcs.df %>%
  #group_by(implant, animal, date) %>%
  #dplyr::summarise(day.pc.pct = mean(signif.si) * 100) %>%
  group_by(implant, animal) %>%
  #dplyr::summarise(pc.pct=mean(day.pc.pct), pc.sem=sem(day.pc.pct)) %>%
  dplyr::summarise(pc.pct = mean(signif.si) * 100) %>%
  ggplot(aes(x=implant, y=pc.pct, group=animal)) +
  geom_point() +
  #geom_errorbar(aes(ymin=pc.pct-pc.sem, ymax=pc.pct+pc.sem), width=0.1) +
  gtheme +
...
plot_grid(
  ...
  plot.pc.stat(field.size.50 / nbins / nbins * 100) +
    ylab('Field size\n(% of maze)')  + ylim(c(0,20)),
#  plot.pc.stat(sparsity)+
#    ylab('Sparsity'),
#  plot.pc.stat(mutual.info) +
#    ylab('Mutual Info\n'),
  plot.pc.stat(spatial.information) + ylab('Spatial Information\n(bits per dF unit)'),
  plot.pc.stat(early.late.cor) + ylab('Field stability (correlation of early vs late session)'),
  nrow = 1)
...
spatial.info.model = lmerTest::lmer(log(spatial.information + 1) ~ implant + (1 | animal), 
...
#wilcox.test(spatial.information ~ implant, data=compared.pcs.df)

print('Field size signif larger in vCA1')
field.size.model = lmerTest::lmer(log(field.size.50)  ~ implant + (1 | animal), data=compared.pcs.df, REML=TRUE)
summary(field.size.model)
anova(field.size.model, refit=FALSE, ddf='Satterthwaite')
plot.model.diagnostics(field.size.model, 
                       compared.pcs.df$animal, 
                       compared.pcs.df$implant)
#wilcox.test(field.size.50 ~ implant, data=compared.pcs.df)
```
...
TODO: compare number of fields at reward to shuffled number of times

Model of cell randomly assigned at distances following observed distribution

```{r}
pc.ids = filter(beforetest.peaks.at.current.rew, signif.si) %>%
...
shuffled.pct.atrew.95p = shuffled.cell.atrew.hist %>%
  dplyr::mutate(few_fields = mid <= 1, many_fields = mid >= 3) %>%
...
X = filter(shuffled.pct.atrew.95p, few_fields) %>% left_join(
  filter(cell.rew.atrew.hist, mid==1)) 
print(dplyr::select(X, implant, npeaks.05p, npeaks.95p, pct.count.cumsum))
...
shuffled.cell.atrew.hist.summary %>%
  ...
  geom_bar(aes(y=m.count),  stat='identity', position='dodge', width=1,
           fill='white', color='grey60', alpha=0.5) +
  geom_errorbar(aes(ymin=m.count-sd.count, ymax=m.count+sd.count), width=0.4, color='grey50') +
  geom_step(data=cell.rew.atrew.hist.summary, aes(x=mid-0.5, y=pct.count), color='blue', size=2) +
...
ggplot(shuffled.cell.dist.hist.summary) +
  geom_bar(aes(x=mid, y=m.count), stat='identity', position='dodge',
           fill='white', color='grey60', width=10, alpha=0.5) +
  geom_errorbar(aes(x=mid, ymin=m.count-sd.count, ymax=m.count+sd.count), color='grey40', width=4) +
