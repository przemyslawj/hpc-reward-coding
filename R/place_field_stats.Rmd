...
mean(goal.cells.df$pct.goal, na.rm=T)
...
pc.test.df = pc.test.df %>%
  ddply(.(date, animal, cell_id), dplyr::mutate,
        min.rew.dist=calc.min.rew.dist(filter.rews.df(beforetest.rewards.df, date[1], animal[1]),
                                       field.max.x[1], field.max.y[1])$rew.dist,
        closest.location.ordinal=calc.min.rew.dist(filter.rews.df(beforetest.rewards.df, date[1], animal[1]),
                                                   field.max.x[1], field.max.y[1])$location.ordinal)

```

Compare with the test trials with habituation trials
```{r warning=FALSE}
baseline.trials.pc = filter(run.trials.pc2rew.dist, day_desc.activity=='habituation day#3')
...
```{r}
baseline.trials.pc = filter(run.trials.pc2rew.dist, day_desc.activity=='habituation day#3', animal!='A-BL')
compared.trials.pc = filter(run.trials.pc2rew.dist, date.activity == format(as.Date(date.reward) - 1), animal!='A-BL')
...
plot.field.locs(filter(pc.test.df, is.pc, day_desc=='learning2 day#1'), 
                'learning2 day#1',
...
plot.field.locs(
  filter(run.trials.pc2rew.dist, day_desc.activity =='habituation day#3', day_desc.reward == 'learning2 day#1'),
  'learning2 day#1') +
  labs(title='Last day habituation')
```

```{r}
plot.field.locs(filter(pc.test.df, day_desc=='learning3 day#1'), 
                'learning3 day#1',
                fields.list=beforetest.fields,
                date.activity.var=date) +
...
plot.field.locs(
  filter(run.trials.pc2rew.dist, day_desc.activity =='habituation day#3', day_desc.reward == 'learning3 day#1'),
  'learning3 day#1') +
  labs(title='Last day habituation')
```

```{r}
plot.field.locs(filter(pc.test.df, day_desc=='learning3 day#3'), 
                'learning3 day#3',
                fields.list=beforetest.fields,
                date.activity.var=date) +
...
plot.field.locs(
  filter(run.trials.pc2rew.dist, day_desc.activity =='habituation day#3', day_desc.reward == 'learning3 day#3'),
  'learning3 day#3') +
  labs(title='Last day habituation')
```
...
plot.field.locs(
  filter(run.trials.pc2rew.dist, day_desc.activity==trial.learning.day, day_desc.reward==reward.learning.day),
  reward.learning.day) + labs(title='Trials at the end of first learning')

plot.field.locs(
  filter(run.trials.pc2rew.dist, day_desc.activity =='habituation day#3', day_desc.reward == reward.learning.day),
  reward.learning.day) +
...
```{r}
summary.habit.trials.pc = run.trials.pc2rew.dist %>%
...
g.pf.change.pct = place.cell.change.pct %>%
  filter(animal != 'A-BL') %>%
  ggplot() +
...
g.pf.shift.fig = plot_grid(g.test.pf.shift,
                           g.pf.change.pct + theme(legend.position = 'top'),
...
Statistical tests for number of goal cells vs number of cells at those locations during habituation
```{r}
