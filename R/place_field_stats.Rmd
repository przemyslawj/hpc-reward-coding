...
pc.test.df %>%
  ...
  dplyr::summarise(mean(npc), mean(pc.pct), mean(signif.pct), sem(signif.pct))
...
summarize.rew.pc.pct = function(df, group.name, dist.var=min.rew.dist, dist.threshold=goal.cell.max.dist) {
...
# Binds two DFs, resulting in a DF with paired rows for each implant, animal, day_desc. Inserts value.var=0 
# where the pair row is absent.
create.df.with.paired.values = function(fst.df, fst.group.name, 
                                        snd.df, snd.group.name, 
                                        value.var=pct.reward) {
  fst.group.name = enquo(fst.group.name)
  snd.group.name = enquo(snd.group.name)
  value.var = enquo(value.var)
  value.varname = quo_name(value.var)
  
  combined.dfs = bind_rows(
    mutate(fst.df, group=quo_name(fst.group.name)),
    mutate(snd.df, group=quo_name(snd.group.name)))  
  
  # Map to wide representation to find grouping rows with missing vals
  wide.df.wo.missing.vals = reshape2::dcast(combined.dfs,
                                            implant + animal + day_desc ~ group,
                                            value.var=value.varname)
    #replace_na(list(!!fst.group.name := 0, !!snd.group.name := 0))
  
  # Map back to narrow representation
  reshape2::melt(wide.df.wo.missing.vals,
                 id.vars=c('implant', 'animal', 'day_desc'),
                 measue_vars=c(quo_name(fst.group.name), quo_name(snd.group.name)),
                 variable.name='group', value.name=value.varname) %>%
    dplyr::mutate(!!value.var:=ifelse(is.na(!!value.var), 0, !!value.var))
}

plot.pc.change.pct = function(pc.change.pct.df) {
  pc.change.pct.df %>%
    ggplot() +
    ...
    xlab('') + ylab('Place cells at reward (%)') + labs(color='')
}

summary.pc.test.df = summarize.rew.pc.pct(filter(pc.test.df, signif.si), 'test')

compared.habit.trials.df = run.trials.pc2rew.dist %>%
  filter(day_desc.activity=='habituation day#3', signif.si) %>%
  dplyr::select(-ends_with('.activity')) %>%
  dplyr::rename(date=date.reward,
                day_desc=day_desc.reward)

test.pc.change.pct = create.df.with.paired.values(
  summarize.rew.pc.pct(compared.habit.trials.df), habituation, 
  summary.pc.test.df, test) %>% filter(animal != 'A-BL') # No test trials for the animal

g.pc.change.pct.test = plot.pc.change.pct(test.pc.change.pct)
g.pc.change.pct.test + ggtitle('Fraction of place cells at reward increased at test trial')
...
When habituation compared to test
```{r}
wilcox.test(pct.reward ~ group, subset(test.pc.change.pct, implant=='vCA1'), paired=TRUE)
wilcox.test(pct.reward ~ group, subset(test.pc.change.pct, implant=='dCA1'), paired=TRUE)
```
...
calc.field.peaks.info = function(day,
                                 ...
                                 rews.df=fst.moved.location.df,
                                 max.rew.dist.thr=20) {
  rew.df = filter.rews.df(rews.df, day, animal_name)
  ...
  cell_names = fields.list[[animal_name]][[day]] %>% names

  map_dfr(cell_names, ~ {
    cell_name = .x
    field = fields.list[[animal_name]][[day]][[paste(cell_name)]]
    ...
    if (nrow(peaks.rowcol) > 0) {
      min.rew.dist.res = calc.min.rew.dist(rew.df, peaks.rowcol[,'row'], peaks.rowcol[,'col'])
    } else {
    ...
    return(list(
      ...
      current.rew.peaks.count = sum(rew.df[location_ordinal %in% loc.ordinals.with.peaks, current_loc], na.rm = TRUE),
      peak2rew.mindist = min(min.rew.dist.res$rew.dist)
    ))
  })
...
create.peaks.df.at.locs = function(fields, locs.df) {
  res = map2_dfr(trial.days$date,
                 trial.days$animal,
                 ~ calc.field.peaks.info(.x, .y, fields, rews.df=locs.df))
  res %>%
...
```{r warning=FALSE}
habit.future.rewards.df = trials.meta.df %>%
  filter(exp=='habituation') %>%
  dplyr::mutate(location_set=2) %>%
  left_join(filter(prev.locations.df, !is_test, !current_loc) %>% dplyr::select(-date),
            by=c('location_set', 'animal'))

habituation.peaks.at.fst.moved = create.peaks.df.at.locs(run.fields, habit.future.rewards.df)
...
```{r}
bind_rows(
  filter(beforetest.peaks.fst.moved, signif.si, npeaks>0),
  filter(trial.peaks.at.fst.moved, signif.si, exp=='habituation')) %>% 
  dplyr::add_count(implant, animal, day_desc, exp, name='ncells') %>%
  filter(ncells > 10) %>%
  dplyr::group_by(implant, animal, day_desc, exp, npeaks) %>%
  dplyr::summarise(peak.pct=n() / mean(ncells) * 100) %>%
  ggplot(aes(x=npeaks, y=peak.pct)) +
  geom_jitter(width=0.1, height = 5) +
  facet_grid(implant ~ day_desc + exp) +
  gtheme


beforetest.multifields.count = bind_rows(
  filter(beforetest.peaks.fst.moved, signif.si, npeaks>0),
  filter(trial.peaks.at.fst.moved, signif.si, exp=='habituation', npeaks>0)) %>%
  dplyr::add_count(implant, animal, day_desc, exp, name='ncells') %>%
  filter(ncells > 10) %>%
  dplyr::group_by(implant, animal, day_desc, exp, is.multi=npeaks > 1) %>%
  dplyr::summarise(multi.pct=n() / mean(ncells) * 100) %>%
  filter(is.multi)
  
#t.test(multi.pct ~ implant, beforetest.multifields.count)
library(lmerTest)
peak.count.model = lmerTest::lmer(multi.pct ~ implant + exp + (1 + implant + exp | animal), data=beforetest.multifields.count, REML=TRUE)
...
trial.peaks.at.rew %>%
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
Distribution of min distance to the first translocated peak
```{r}
baseline.peaks.fst.moved = filter(trial.peaks.at.fst.moved, day_desc=='habituation day#3', signif.si)

ggplot() +
  geom_density(data=baseline.peaks.fst.moved, aes(x=peak2rew.mindist), linetype='dashed') +
  geom_density(data=filter(beforetest.peaks.fst.moved, signif.si, day_desc == 'learning2 day#1'), aes(x=peak2rew.mindist)) +
  #geom_density(data=filter(trial.peaks.at.fst.moved, signif.si, day_desc == 'learning1 day#5'), aes(x=peak2rew.mindist)) +
  #facet_wrap(animal ~ ., scales='free_x') +
  facet_grid(. ~ implant) +
  geom_vline(xintercept = goal.cell.max.dist, linetype='dotted') +
  coord_flip() +
  gtheme +
  xlab('Field distance from the first translocated reward (%)') + ylab('Density')
```
...
wilcox.test(current.multireward.pct ~ implant, day.peak.summary, subset=day_desc == 'learning2 day#1')
