...
- do on all cells not only place cells?
- empirical distribution per mouse per location set.
```{r}
cell.dist.df = compared.test.trials.pc %>%
  dplyr::select(implant, animal, date, cell_id, min.rew.dist) %>%
...
shuffled.cell.dist.summary = shuffled.cell.dist.df %>%
  ...
  dplyr::summarise(m.dist = median(min.rew.dist), 
                   n=n(), 
...
TODO 
- do for field closer to reward not max field
- include all cells that were signif one day? or on first day?
```{r}
hist.breaks = seq(0, 120, 10)

create.hist.tibble = function(vals, breaks=hist.breaks) {
...
shuffled.cell.dist.hist.summary = shuffled.cell.dist.summary %>%
  group_by(implant, animal, shuffle.id) %>%
  dplyr::summarise(create.hist.tibble(m.dist)) %>%  # histogram per shuffle
  group_by(animal, implant, mid) %>%
  dplyr::summarise(m.count=mean(pct.count), sem.count=sem(pct.count), sd.count=sd(pct.count))

cell.rew.distance.summary = compared.test.trials.pc %>%
  dplyr::group_by(implant, animal, cell_id) %>%
  dplyr::summarise(m.dist = median(min.rew.dist), 
                   n=n(), 
                   var.dist=var(min.rew.dist) / (n),
                   at.rew.times=sum(min.rew.dist <= goal.cell.max.dist)) %>%  # biased estimate for variance
  filter(n>1) %>%
  dplyr::mutate(group='actual', line_g=group)

cell.rew.distance.hist.summary = cell.rew.distance.summary %>%
  group_by(implant, animal) %>%
  dplyr::summarise(create.hist.tibble(m.dist))

cell.rew.distance.summary %>%
  group_by(implant, animal) %>%
  dplyr::summarise(create.hist.tibble(at.rew.times, breaks=seq(-0.5,3.5,1))) %>%
  ggplot() +
  geom_col(aes(x=mid, y=count)) +
  facet_wrap(. ~ animal + implant) +
  gtheme

ggplot(shuffled.cell.dist.hist.summary) +
  geom_col(aes(x=mid, y=m.count), fill='grey50', color='grey50', width=10, alpha=0.5) +
  geom_errorbar(aes(x=mid, ymin=m.count-sd.count, ymax=m.count+sd.count), color='grey30', width=4) +
  geom_col(data=cell.rew.distance.hist.summary, aes(x=mid, y=pct.count), fill='blue', color='blue', width=10, alpha=0.5) +
  #geom_histogram(data=cell.rew.distance.summary, aes(x=m.dist), fill='blue', breaks=hist.breaks, alpha=0.5) +
...
#bind_rows(
#  cell.rew.distance.summary,
#  shuffled.cell.dist.summary) %>%
#  ggplot(aes(x=m.dist, y=var.dist, group=group)) +
#  geom_point(style=1) +
#  facet_grid(group ~ implant) +
#  gtheme
```
...
#   filter(signif.si, date.activity == format(char2date(date.reward) - 1)) %>%
#   dplyr::rename(date=date.reward,
#                 day_desc=day_desc.reward) %>%
