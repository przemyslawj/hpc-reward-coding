...
summary(fem.vca1)
plot.model.diagnostics(fem.dca1, vca1.rew.pc.pct$animal, vca1.rew.pc.pct$exp)

g.pc.change.pct.test = 
  plot.val.change.between.exp(merged.summary.df, pct.reward) +
...
habit.fields.at.test.rews = map_dfr(1:nrow(habittest.daypair), ~ {
  ...
  calc.field.peaks.info(activity_date,
                        ...
                        run.fields, 
                        rew.df=rew.df) %>%
    dplyr::mutate(date.activity = activity_date,
                  day_desc.activity = habittest.daypair$day_desc[.x],
...
habit.fields.at.test.rews = habit.fields.at.test.rews %>%
  ...
  dplyr::rename(day_desc.reward = day_desc) %>%
  left_join(place.cell.db, by=c('animal'='animal', 'date.activity'='date', 'cell_id'='cell_id', 'day_desc.activity'='day_desc'))
```
...
habit.summary.fields.pct.df = summarize.rew.pc.pct(
  ...
  dist.threshold=15) %>% dplyr::ungroup()
