...
```{r}
source('sim_rew_distances.R')
xycoords = sim.xy.coord(nplace.fields=100)
```

```{r}
rews.df = filter(test.rewards.df, day_desc=='learning2 day#1')
rew.distances.df = calc.rew.distances.df(rews.df, xycoords$field.x, xycoords$field.y)
```
...
rew.distances.df %>%
  ...
  ggplot2::coord_flip()
```
For each animal what distance do 15 % of cells are
```{r}
rew.distances.df %>%
  dplyr::group_by(animal) %>%
  dplyr::summarise(quantile15.distance=quantile(min.rew.dist, 0.15)[[1]]) %>%
  dplyr::summarise(mean(quantile15.distance))
```
...
TODO:
- is the distribution of the place fields different than occupancy guided?
- how does dist of place fields compare with the uniform distribution
