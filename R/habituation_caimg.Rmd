...
first_learning_day = char2date('2019-08-27')
...
cells = unique(data$cell_id) %>% sort
...
for (i in 1:length(days)) {
  for (cell_name in cells) {
    ...
    cell.df = filter(data, cell_id == cell_name, date == day.str) 
    cell.df$trial = as.factor(cell.df$trial)
    cell.events = filter(cell.df, nevents > 0) 
    if (nrow(cell.events) > 0) {
      cell.df.run = filter(cell.df, velocity >= run.threshold)
      trace.col = 'deconv_trace'
      pf = getPlaceField(cell.df.run, trace.col)
      trace.pf = getPlaceField(cell.df.run, 'trace')
      entropy = pf$entropy
      ...
      pci.df = bind_rows(pci.df, data.frame(cell_id=cell_id, day=day.str, 
                                            ...
                                            spatial.information.trace=trace.pf$spatial.information,
                                            spatial.information.trace.perspike=trace.pf$spatial.information.perspike,
      ...
      trace.quantiles = quantile(cell.df.run[, trace.col], probs=c(0.2, 0.9))
      ...
      ggsave(paste0(gen_imgs_dir, 'place_field_cell_', cell_name, '.jpg'), g.placefield,
             width=4.0, height=3.5, units='cm')
    }
  }
}

pci.df = arrange(pci.df, desc(spatial.information))
datatable(pci.df) %>% 
  formatRound(c('spatial.information', 'spatial.information.perspike', 
                'spatial.information.trace', 'spatial.information.trace.perspike',
                'entropy', 'event_spread'),digits = 2) %>%
...
pci.df = mutate(pci.df, cell_desc=paste0('Cell ', cell_id, 
                                        ' SI=', round(spatial.information,1), 
...
```{r}
day.str = '2019-08-28'
cells.selected = c(3,4)
trial.df = filter(data, cell %in% cells.selected, date==day.str, exp_title == 'trial')
trial.df$cell_name=as.factor(trial.df$cell) %>% as.integer
trial.events.df = filter(trial.df, nevents > 0)
g = ggplot(trial.df) +
  geom_line(aes(x=timestamp/1000, y=trace, group=1)) +
  geom_point(data=trial.events.df, aes(x=timestamp/1000, y=trace + 10), shape=8, colour='red') +
  facet_grid(cell_name ~ trial_id) +
  gtheme +
  xlab('Time (sec)') +
  ylab('z-scored dF/F') +
  xlim(c(0,20))
...
```{r}
ggplot(trial.df) +
  geom_line(aes(x=timestamp/1000, y=deconv_trace, group=1)) +
  geom_point(data=trial.events.df, aes(x=timestamp/1000, y=deconv_trace + 1), shape=8, colour='red') +
  facet_grid(cell_name ~ trial_id) +
  gtheme +
  xlab('Time (sec)') +
  ylab('z-scored dF/F') +
