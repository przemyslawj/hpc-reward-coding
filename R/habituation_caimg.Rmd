...
```{r}
data_dir = '/mnt/DATA/Prez/cheeseboard/2019-08/2019-08-habituation/'
#data_dir = '/mnt/DATA/Prez/cheeseboard/2019-07-learning/'
subject = 'E-BL'
datestr = '2019-08-28'
ca_img_result_dir = paste0(data_dir, datestr, '/trials/mv_caimg/', subject, '/')
gen_imgs_dir = '/tmp/'
...
```{r}
data.events = data %>%
  select('date', 'timestamp', 'trial', starts_with('events_')) %>%
...
data = left_join(data.traces, data.events, by=c('date', 'trial', 'timestamp', 'cell'))
...
data = data %>%
  ddply(.(date, trial, cell), plyr::mutate,
        timestamp = timestamp,
