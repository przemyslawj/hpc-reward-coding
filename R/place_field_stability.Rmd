...
# Stability during test trial
```{r}

field.stability = function(binned.cell.df, make.plot=TRUE) {
  #half.session.len = floor(nrow(binned.cell.df) / 2)
  #early.cell.df = binned.cell.df[1:half.session.len,]
  #late.cell.df = binned.cell.df[(half.session.len+1):nrow(binned.cell.df),]
  half.session.timestamp = min(100 * 1000, max(binned.cell.df$timestamp) / 2)
  end.timestamp = 2 * half.session.timestamp
  early.cell.df = binned.cell.df[timestamp <= half.session.timestamp,]
  late.cell.df = binned.cell.df[timestamp > half.session.timestamp & timestamp < end.timestamp,]
  
  early.pf = calcPlaceField(bin_x = early.cell.df$bin.x, 
                            bin_y = early.cell.df$bin.y, 
                            nbins_x = 20, 
                            nbins_y = 20, 
                            trace = early.cell.df$deconv_trace, 
                            binnedTrace = early.cell.df$response_bin, 
                            minOccupancy = 1, 
                            kernelSize = 7, 
                            gaussianVar = 1.4)
  
  late.pf = calcPlaceField(bin_x = late.cell.df$bin.x, 
                           bin_y = late.cell.df$bin.y, 
                           nbins_x = 20, 
                           nbins_y = 20, 
                           trace = late.cell.df$deconv_trace, 
                           binnedTrace = late.cell.df$response_bin, 
                           minOccupancy = 1, 
                           kernelSize = 7, 
                           gaussianVar = 2.0)
  
  early.pf.df = create.pf.df(early.pf$field, early.pf$occupancy)
  late.pf.df = create.pf.df(late.pf$field, late.pf$occupancy)
  if (make.plot) {
    g.fields = plot_grid(
      plot.trace.events(early.cell.df, event.var=nevents, event.thr=1) +
        geom_rewards(test.rewards.df, animal_name, day, 100, rew.colours = c('current_rew'='black')),
      plot.trace.events(late.cell.df, event.var=nevents, event.thr=1) +
        geom_rewards(test.rewards.df, animal_name, day, 100, rew.colours = c('current_rew'='black')),
      plot.pf(early.pf.df, nbins, nbins) +
        geom_rewards(test.rewards.df, animal_name, day, 20, rew.colours = c('current_rew'='black')),
      plot.pf(late.pf.df, nbins, nbins) +
        geom_rewards(test.rewards.df, animal_name, day, 20, rew.colours = c('current_rew'='black')),
      ncol=2)
  }
  
  earlylate.cor = field.cor(early.pf.df, late.pf.df, make.cor.plot=TRUE, max.xy=nbins)
  
  g = NA
  if (make.plot) {
    earlylate.cor.title = paste0('Early vs Late cor=', format(earlylate.cor$cor, digits=2))
    g.cor = earlylate.cor$g + labs(title=earlylate.cor.title, fill='Correlation')
    
    g = plot_grid(g.fields, g.cor, ncol=2, rel_widths = c(2, 1.1))
  }
  
  return(list(cor=earlylate.cor$cor, g=g))
}
```

Example

Test trial place field stability
```{r}
...
binned.traces.run = prepare.run.dirtraces(data.traces[exp_title == 'beforetest',], 
                                          nbins, 
...
```{r}
cell_name = 230
binned.cell.df = binned.traces.run[cell_id == cell_name & x >= 0 & y >= 0, ]

field.stability(binned.cell.df, make.plot = TRUE)
```


# Quantify stability of place fields over the test trial duration.
```{r}
test.trials.meta = pc.test.df %>% 
  group_by(animal, date) %>% 
  dplyr::select(implant, animal, date, day_desc) %>%
  dplyr::distinct()

pc.test.df$early.late.cor = rep(-1.0, nrow(pc.test.df))
make.plots = FALSE
pc.test.df = as.data.table(pc.test.df)
gen_imgs_dir = "/mnt/DATA/Prez/pf_stability-beforetest/"

for (i in 1:nrow(test.trials.meta)) {
  animal_name = test.trials.meta$animal[i]
  day = test.trials.meta$date[i]
  caimg_result_dir = find.caimg.dir(caimg_result_dirs, animal_name, day)
  ...
  img.dir = file.path(gen_imgs_dir, animal_name, day)
  dir.create(img.dir, showWarnings=FALSE, recursive=TRUE)
  for (cell_name in pc.test.df[animal==animal_name & date==day, cell_id]) {
    binned.cell.df = binned.traces.run[cell_id == cell_name, ]
    cell.stability.res = field.stability(binned.cell.df, make.plot=make.plots)
    pc.test.df[animal==animal_name & date==day & cell_id == cell_name, early.late.cor:= cell.stability.res$cor]
    if (make.plots) {
      plot.title = paste0(animal_name, ' ', day, ', cell:', cell_name,
                         ifelse(pc.test.df[animal==animal_name & date==day & cell_id == cell_name, signif.si], ' SI', ''))
      ggsave(file.path(img.dir, paste0(day, '_', animal_name, '_', cell_name, '.jpg')),
             cell.stability.res$g + draw_label(plot.title, x=0.3, y=0.97, size=10),
             units = 'cm',
             width = 17,
             height=6)
    }
...
Compared early vs late correlations: same number of stable place cells during the test
```{r}
signif.pc.test.df = filter(pc.test.df, signif.si) 
  #dplyr::mutate(animal_beforetest_day = paste(animal, day_desc, sep='_') %>% str_replace(' test', '')) %>% 
  #filter(animal_beforetest_day %in% kept.testtrials$animal_beforetest_day)

group_by(signif.pc.test.df, implant, animal) %>%
  dplyr::summarise(mean(early.late.cor, na.rm=T), sem(early.late.cor), median(early.late.cor, na.rm=T))

ggplot(signif.pc.test.df, aes(x=implant, y=early.late.cor)) +
  geom_violin() +
  stat_summary(fun.y=median, geom="point", shape=23, size=2) +
  gtheme

stability.model = lmer(early.late.cor ~ implant + (1 | animal), data=signif.pc.test.df, REML=TRUE)
summary(stability.model)
plot.model.diagnostics(stability.model, 
                       signif.pc.test.df$animal, 
                       signif.pc.test.df$implant)
```

Compare within-day stability of place cells during habituation: not different
```{r}
habit.place.cell.db = filter(place.cell.db, signif.si, exp == 'habituation', day_desc == 'habituation day#3')
group_by(habit.place.cell.db, implant, animal) %>%
  dplyr::summarise(mean(early.late.cor, na.rm=T), sem(early.late.cor), median(early.late.cor, na.rm=T))

ggplot(habit.place.cell.db, aes(x=implant, y=early.late.cor)) +
  geom_violin() +
  stat_summary(fun.y=median, geom="point", shape=23, size=2) +
  gtheme

stability.model = lmer(early.late.cor ~ implant + (1 | animal), data=habit.place.cell.db, REML=TRUE)
summary(stability.model)
plot.model.diagnostics(stability.model, 
                       habit.place.cell.db$animal, 
                       habit.place.cell.db$implant)
```
...
```{r}
run.trials.si %>%
  ggplot(aes(x=day_desc,y=spatial.information)) +
  geom_violin(fill='grey80') + 
  stat_summary(fun.y=median, geom="point", shape=23, size=2) +
  gtheme +
  facet_grid(. ~ implant) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(y='Spatial info', x='', color='Place cell\non both days')
```

```{r}
wilcox.test((spatial.information) ~ exp, data=run.trials.si)
```
...
pf.diff.df %>%
  ...
  ggplot() +
  geom_violin(aes(x=day_desc, y=si.change)) +
  geom_hline(aes(yintercept=0), linetype='dashed') +
  gtheme +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
```{r}
max.xy = getNBinsXY()
all.days = unique(run.trials.si$date) %>% char2date()
  ...
  for (i in 1:length(all.days)) {
    ...
    for (animal_name in unique(df$animal)) {
      ...
      day.trials %>%
        ...
        select(-field.max.x.diff, -field.max.y.diff)-> pc.change
...
```{r}
field.cor.thr=0.4
stable.cell.db = early.late.comparison %>%
  mutate(is.stable=replace_na(field.cor >= field.cor.thr, FALSE)) %>%
  select(date, animal, cell_id, is.stable)
```

Do place cells have stable field peaks from early to late trials
```{r}
#left_join(early.late.comparison, place.cell.db, by=c('date', 'animal', 'cell_id')) %>%
early.late.comparison %>%
  filter(is.pc) %>%
    group_by(animal, date) %>%
    sample_n(50, replace = TRUE) %>%
    ungroup() %>%
  ggplot(aes(x=date, y=field.cor)) +
  geom_violin(fill='grey80') +
  geom_hline(aes(yintercept=0), linetype='dashed') +
  gtheme +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(x='Date', y='Field correlations') +
  stat_summary(fun.y=median, geom="point", shape=23, size=2)
```
