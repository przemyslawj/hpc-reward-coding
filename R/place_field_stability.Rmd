...
```{r}
bind_rows(odd.even.comparison, early.late.comparison) %>%
  filter(spatial.info.mean > 0) %>%
  mutate(is.place.cell = spatial.info.mean >= 0) %>%
  ggplot(aes(x=spatial.info.mean, y=field.cor)) +
  ...
  ylab('Correlation of place field between trials') +
  labs(color='Place cell', title='Early vs Late, Odd vs Even')
```
...
day.comparison.mean = day.comparison.mean %>%
  mutate(is.place.cell = m.signif.si == 1 & 
...
day.comparison.mean  %>%
  ggplot(aes(x=m.spatial.info, y=m.field.cor, color=is.place.cell)) +
  geom_point(alpha=0.2, size=1) +
  geom_rug(col=rgb(.5,0,0,alpha=.009)) +
  geom_vline(xintercept=si.thresh, linetype='dashed') +
  geom_hline(yintercept=field.min.cor, linetype='dashed') +
...
pc.rew.dist.df = merged.run.trials.pc %>%
  filter(!is.na(location_set), is.place.cell) %>%
