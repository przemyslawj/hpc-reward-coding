...
odd.even.comparison %>%
  ggplot() +
  geom_violin(aes(x=date, y=field.cor))
```
...
run.trials.si.field %>%
  ggplot() +
  geom_point(aes(x=spatial.information, y=m.field.cor, color=signif.si), alpha=0.2) +
  gtheme +
  xlab('Spatial information (bits/s)') +
  ylab('Field mean correlation within day') +
  labs(color='Signif SI')
```
...
pc.rew.dist.df = merged.run.trials.pc %>%
  ...
  ddply(.(date,animal,cell_id), mutate, 
        ...
        #min.rew.dist=calc.min.rew.dist(date,animal,field.max.x, field.max.y))
