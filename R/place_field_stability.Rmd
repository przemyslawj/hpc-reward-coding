...
cell.spatial.info = function(cell.df, generate.plots=FALSE, nshuffles=0) {
  ...
  trace.quantiles = quantile(trace.vals, c(0.85, 0.95, 1.0)) %>% unname + 0.01
  pf = with(cell.df, getCppPlaceField(smooth_trans_x, smooth_trans_y, trace.vals, 
                                      trace.quantiles, trial_ends, nshuffles, frame.hz/2))
  
  si.signif.thresh = quantile(pf$shuffle.si, 0.95)[[1]]
  si.signif = pf$spatial.information >= si.signif.thresh
  mi.signif.thresh = quantile(pf$shuffle.mi, 0.95)[[1]]
  ...
  cell_info = list(cell_id=cell_name,
                   ...
                   quantile20=trace.quantiles[[1]],
                   quantile90=trace.quantiles[[2]],
                   #entropy=pf$entropy,
