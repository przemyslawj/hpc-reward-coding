...
```{r}
run.trials.si %>%
  filter(date < '2019-09-08') %>%
  ggplot(aes(x=date,y=mutual.info - mutual.info.bias)) +
  geom_violin(fill='grey80') + 
  stat_summary(fun.y=median, geom="point", shape=23, size=2) +
  gtheme +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(y='Mutual information w.o. bias', x='', color='Place cell\non both days')
```

```{r}
wilcox.test((mutual.info - mutual.info.bias) ~ (as.Date(date) < fst.learning.day), data=run.trials.si)
```
