...
mhab_loc = hab_loc %>%
  ...
  dplyr::summarise(mnorm_crossings = mean(norm_crossings) * 2,
...
test.trial.df.joined = test.trial.df %>%
  select(animal, location_set, date, norm_crossings, crossings_0_60s) %>%
  mutate(type="test")  %>%
...
test.trial.df.joined = test.trial.df.joined %>%
  mutate(paired_group=interaction(date, type, lex.order=TRUE),
         group_id=interaction(date, animal))
...
gcross.compare = ggplot(test.trial.df.joined, aes(x=paired_group_num, y=norm_crossings)) +
  ...
  geom_point(size=1, aes(color=animal)) +
...
gcross.paired.absolute = test.trial.df.joined %>%
  select(animal, date, location_set, crossings_0_60s, type) %>%
...
Future crossings
```{r}
future.crossings.df = filter(d, exp_title=='beforetest', !is.na(ncross_future_roi))
future.crossings.df %>%
  group_by(learning_day1, animal) %>%
...
puff.d %>%
  ...
  #geom_jitter(aes(color=animal),height=0, width=0.1)
  stat_summary(fun.data = "mean_cl_boot", geom='point', mapping=aes(group=animal, color=animal))
