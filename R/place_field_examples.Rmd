...
geom_rewards = function(rewards.df, subject=NULL, day=NULL, nbins=20) {
  ...
  rew.colours = c('prev_rew'='gray60', 'current_rew'='black', 'TRUE'='red', 'FALSE'='blue')
  #rew.shape = 0
  rew.shape = 2
  rew.size = 2
...
cells.traces %>%
  ...
  geom_point(data=cells.events, mapping=aes(x=timestamp/1000, y=140), color='red', size=0.7) +
  facet_grid(cell_id ~ .) +
  gtheme +
...
create.all.day.pf = function(selected.cells.df, normalize.vals=FALSE) {
  pf = selected.cells.df %>%
    adply(.margins=1, .fun=rowwise.partial.df) %>%
    dplyr::rename(date=day)
  
  if (normalize.vals) {
    max.val = max(pf$value.conv)
    pf$value.conv = pf$value.conv / max.val
  }
  return(pf)
}
...
```
Multicell plot of several day activity
```{r}
rewards.df = as.data.table(rewards.df)
animal_name = 'E-TR'

day.normalize.vals = function(pf.df) {
  pf.df %>%
    group_by(animal, date, day_desc, cell_id) %>%
    dplyr::mutate(value.conv = value.conv / max(value.conv))
}

g.etr = bind_rows(
  create.all.day.pf(create.selected.pf.df(animal_name, 89), F),
  create.all.day.pf(create.selected.pf.df(animal_name, 90), F),
  create.all.day.pf(create.selected.pf.df(animal_name, 99), F),
  create.all.day.pf(create.selected.pf.df(animal_name, 122), F),
  create.all.day.pf(create.selected.pf.df(animal_name, 139), F),
  ) %>% 
...
animal_name = 'G-BR'
g.gbr = bind_rows(
  create.all.day.pf(create.selected.pf.df(animal_name, 46), T),
  create.all.day.pf(create.selected.pf.df(animal_name, 47), T),
  create.all.day.pf(create.selected.pf.df(animal_name, 102), T),
  create.all.day.pf(create.selected.pf.df(animal_name, 107), T),
  create.all.day.pf(create.selected.pf.df(animal_name, 139), T),
  ) %>% 
  filter(stringr::str_starts(day_desc, 'learning'), day_desc != 'learning3 day#3') %>%
  plot.pf(max.x=nbins, max.y=nbins) +
    facet_grid(cell_id ~ day_desc) +
