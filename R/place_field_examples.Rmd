...
plot.rootdir = "/mnt/DATA/Prez/pf-animal-testdays_smooth-v3/"

create.multifields.loc.df = function(...) {
  input.row = tibble(...)
  field = beforetest.fields[[input.row$animal]][[input.row$date]][[format(input.row$cell_id)]]
  peaks.rowcol = as.data.frame(fieldPeaks(field, min.peakheight=0.5, sigma=1.4))
  peaks.rowcol %>% 
...
for (animal_name in names(run.fields)) {
  ...
  for (chunk_i in seq_along(cell_chunks)) {
    ...
    g.chunk = ggplot() +
      ...
      geom_point(data=day.peaks.df, mapping=aes(x=row, y=nbins-col+0.7), color='black', size=2, alpha=0.5) +
      theme(legend.position = 'top')
