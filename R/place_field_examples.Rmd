...
g.etr = map_dfr(c(137, 1, 54, 295, 363, 6), ~ create.animal.days.df(animal_name, .x)) %>%
  create.all.day.pf() %>%
...
Multicell plot for all cells, all days per cell / experiment
```{r}
rewards.df = as.data.table(rewards.df)
place.cell.db = data.table(place.cell.db)
pc.test.df = data.table(pc.test.df)
...
trials.meta.df = data.table(trials.meta.df)
plot.rootdir = "/mnt/DATA/Prez/pf-animal-alldays/"
#plotted.trial.days = c('habituation day#3')
plotted.trial.days = c()
...
for (animal_name in names(run.fields)) {
  ...
  chunk_size = 25
  if (length(plotted.trial.days) == 0) {
    plotted.trial.days = unique(rewards.df$day_desc)
  }
  cell_ids = union(
    place.cell.db[animal==animal_name & day_desc %in% plotted.trial.days, cell_id] %>% unique,
    pc.test.df[animal==animal_name, cell_id] %>% unique) %>% 
    sort
  cell_chunks = split(sort(cell_ids), ceiling(seq_along(cell_ids)/chunk_size))
  
  merged.rewards.df = bind_rows(filter(rewards.df, animal == animal_name, day_desc %in% plotted.trial.days), 
  ...
  for (chunk_i in seq_along(cell_chunks)) {
    ...
    selected.cells.days.df = map_dfr(cell_chunk, ~ create.animal.days.df(animal_name, .x)) %>%
      filter(day_desc %in% plotted.trial.days)
    ...
    g.chunk = ggplot() +
      geom_rect(data=beforetest.days.pf, xmin=0, xmax=100, ymin=0, ymax=100, fill='#cccccc') +
    ...
    if (nrow(day.peaks.df) > 0) {
      g.chunk = g.chunk +     
        geom_point(data=day.peaks.df, mapping=aes(x=pmax(0,row - 0.5), y=pmax(0, nbins-col) + 0.2), 
...
Plot selected multi fields during test trials
```{r}
animal_name = 'K-BR'
#selected.cells =  c(149, 172, 131, 153, 75)
#selected.cells =  c(43, 233, 101, 223, 90)
#selected.cells = c(79, 101, 21, 178, 136, 117)
selected.cells = c(61, 108, 39, 25, 120, 114)
g.gbr = map_dfr(selected.cells,
  ...
  plot.pf(max.x=nbins, max.y=nbins) +
    facet_grid(cell_id ~ day_desc) +
    geom_rewards(beforetest.rewards.df, animal_name, nbins=nbins, 
                 rew.colours = list('prev_rew'='white', 'current_rew'='white')) +
  geom_maze_contour(diameter = nbins) +
  theme(legend.position = 'none')
  
g.gbr
ggsave('/tmp/pf_k-br.pdf', plot = g.gbr, width = 7, height = 11, units = 'cm', dpi = 300, device = cairo_pdf)
```
...
Plot selected multi fields during habituation
```{r}
g.gbr = map_dfr(selected.cells, 
...
animal.caimg_dirs = Filter(
  function(caimg_dir) {
    str_detect(caimg_dir, animal_name) & str_detect(caimg_dir, 'habituation') & str_detect(caimg_dir, '2019-08-29')
  },
  caimg_result_dirs)

paths.df.list = list()
binned.events.list = list()
for (caimg_dir in animal.caimg_dirs) {
  ...
  if (nrow(data.traces) > 0) {
    ...
    data.traces = add.running.col(data.traces, 3.3, 10)
    data.traces$date = char2date(data.traces$date[1])
    binned.traces = bin.time.space(data.traces, 
                                   ...
                                   binned.var='deconv_trace',
                                   timebin.dur.msec=200)
    binned.events.list[[caimg_dir]] = binned.traces[nevents >= 0.2, ]
    
    cell_name = binned.traces$cell_id[1]
    paths.df.list[[caimg_dir]] = binned.traces[cell_id==cell_name, ]
  }
}
binned.events = rbindlist(binned.events.list)
paths.df = rbindlist(paths.df.list)
trials.meta.df$date = as.IDate(trials.meta.df$date)  # This is a workaround for non-matching types
#binned.events$date = char2date(binned.events$date)
#paths.df$date = char2date(paths.df$date)
binned.events = left_join(binned.events, trials.meta.df, by=c('animal', 'date'))
...
```
Save only event maps for selected cells
```{r}
event.thr=0.5
day.events.df = binned.events[day_desc == 'habituation day#3' & cell_id %in% sampled.cells & is_running & nevents >= event.thr]
all.paths.df = map_dfr(sampled.cells, ~ mutate(paths.df[day_desc == 'habituation day#3'], cell_id=.x))
g = ggplot(day.events.df) +
  geom_point(aes(x=x, y=100-y), color='red', size=0.5) +
  xlim(c(0, 100)) + ylim(c(0, 100)) +
  geom_path(data=all.paths.df, mapping=aes(x=x, y=100-y, group=trial_id), alpha=0.2) +
  facet_grid(cell_id ~ .) +
  theme_void()
g
ggsave(paste0('/tmp/pf_habit_', animal_name, '_events.pdf'), 
       width = 3, height = 12, units = 'cm', dpi = 300, device = cairo_pdf)
```
