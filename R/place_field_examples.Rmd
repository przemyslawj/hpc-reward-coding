...
#data.traces$is_running = isRunning(data.traces, 2, 4, 500)
 ...
 data.traces$is_running = isRunning(data.traces, 1.6, 3.3, 500) 
...
Plot event vectors
```{r}
binned.cells.df = bin.time.space(data.traces, 
                                 nbins.x = nbins,
                                 nbins.y = nbins,
                                 get.bin.thresholds.fun=get.quantiles.fun(c(0.95, 1.0)),
                                 binned.var='trace',
...
g.events = plot.event.vectors(binned.cells.df[cell_id %in% selected.cell_ids], event.var=nevents, event.thr=1) +
...
for (cell_name in selected.cell_ids) {
  pf = cell.spatial.info(binned.cells.df[is_running & cell_id == cell_name & exp_title == 'beforetest', ],
                         nbins, nbins,
...
trials.meta.df = data.table(trials.meta.df)
plot.rootdir = "/mnt/DATA/Prez/pf-animal-testdays_smooth-v3-test/"
...
```{r}
animal_name = 'G-BR'
g.gbr = map_dfr(c(46, 47, 102, 107, 139), ~ create.animal.days.df(animal_name, .x)) %>%
  create.all.day.pf() %>%
  filter(stringr::str_starts(day_desc, 'learning'), day_desc != 'learning3 day#3') %>%
  day.normalize.fields() %>%
  plot.pf(max.x=nbins, max.y=nbins) +
