...
```{r}
animal_name = 'K-BR'
g.gbr = map_dfr(c(36, 59, 62, 72, 122), 
...
g.gbr
ggsave('/tmp/pf_kbr.pdf', plot = g.gbr, width = 7, height = 10, units = 'cm', dpi = 300, device = cairo_pdf)
```
...
ggsave(paste0('/tmp/pf_habit_.', animal_name, '.pdf'), 
       plot = g.gbr, width = 3, height = 12, units = 'cm', dpi = 300, device = cairo_pdf)
```
...
animal.caimg_dirs = Filter(
  function(caimg_dir) {
    str_detect(caimg_dir, animal_name) & str_detect(caimg_dir, 'habituation') & str_detect(caimg_dir, '2020_10_12')
  },
  caimg_result_dirs)
