...
rowwise.cell.desc = function(row) {
  ...
  if (nrow(goal.db.row) > 0) {
    row$desc = paste0(ifelse(goal.db.row$signif.si, 'PC ', ''), ifelse(goal.db.row$is.goal.cell, ' & GC', ''))
  }
  row
}
...
plot.rootdir = "/mnt/DATA/Prez/pf-animal-occupancy1_5s/"
...
for (animal_name in names(run.fields)) {
  ...
  merged.results.df = bind_rows(filter(rewards.df, animal == animal_name), 
                                filter(test.rewards.df, animal == animal_name) %>% 
  ...
  for (chunk_i in seq_along(cell_chunks)) {
    ...
    g.chunk = ggplot() +
      ...
      facet_grid(cell_id ~ day_desc) +
      geom_text(data=cell.desc.df, mapping=aes(x=1, y=nbins - 2, label=desc), size=3, hjust=0) +
      geom_rewards(merged.results.df, animal_name) +
      theme(legend.position = 'top')
